...
from PyQt5.QtWidgets import QApplication, QLabel, QWidget, QLineEdit, QPushButton, QVBoxLayout, QHBoxLayout, QSizePolicy, QShortcut
...
rotation_update_needed = 0

ser = serial.Serial(port = '/dev/tty.usbmodem11401', baudrate = 9600)
        ...
        if(theremin_go):
            if(theremin_zoom_mode):
                if (theremin_direction == 'x'):
                    ...
                    window.rotation_updated.emit(rotation_scale_factorz, 'z')
...
class MainWindow(QWidget):
    ...
    rotation_updated = pyqtSignal(float, str)
    ...
    def __init__(self):
        ...
        self.rotation_updated.connect(self.display_rotation)
    ...
    def on_button_clicked(self):
        ...
        for program in vals.split('<'):
            if (len(program.splitlines()) > 256):
                self.timer.label.setText(f'Error: Function too complex')
                return
